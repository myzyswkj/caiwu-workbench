 ...
 *   - 应收：别人欠我（客户货款等）
 *   - 应付：我欠别人（供应商等）
 *   - 预付：我预付给对方、待收回 / 待核销的款项（其未用完余额可在「项目核算」中
 *           按关联项目汇总为「应收回款项」）
 * 核销时可一键登记到「登记内账」流水，实现业务→财务联动
 * 每条往来可填写「关联项目」，用于与「项目核算」联动。
 * ============================================================ */
(function (global) {
  'use strict';
  var FW = global.FW;
  ...
  function render() {
    ...
    var preA = list.filter(function (r) { return r.kind === '预付'; }).reduce(function (a, r) { return a + balance(r); }, 0);
    ...
    document.getElementById('content').innerHTML =
      '<div class="stat-row">' +
        ...
        '<div class="stat"><div class="label">预付余额（待收回/核销）</div><div class="value expense">' + FW.fmtMoney(preA) + '</div></div>' +
        '<div class="stat"><div class="label">净头寸（应收+预付−应付）</div><div class="value">' + FW.fmtMoney(recA + preA - payA) + '</div></div>' +
      '</div>' +
      '<div class="card"><h3>账龄分析 <span class="sub">未结清合计 ' + FW.fmtMoney(totalOut) + '</span></h3>' + agingHtml + '</div>' +
      '<div class="card"><h3>往来明细</h3>' + (list.length ? tableHtml(list) : '<div class="empty">还没有往来账，点右上角「＋ 新增往来」登记客户 / 供应商的应收应付 / 预付。</div>') + '</div>';
  ...
  function tableHtml(list) {
    var trs = list.map(function (r) {
      ...
      var kindTag = r.kind === '应收' ? '<span class="tag expense">应收</span>' : (r.kind === '应付' ? '<span class="tag income">应付</span>' : '<span class="tag prepay">预付</span>');
      ...
      return '<tr>' +
        ...
        '<td class="num ' + (b > 0 ? (r.kind === '应付' ? 'income' : 'expense') : '') + '"><b>' + FW.fmtMoney(b) + '</b></td>' +
    ...
    return '<table><thead><tr><th>对方</th><th>项目</th><th>类别</th><th>类型</th><th class="num">金额</th><th class="num">已收/付</th><th class="num">余额</th><th>账龄</th><th>状态</th><th>操作</th></tr></thead><tbody>' + trs + '</tbody></table>';
  }

  function openForm(id) {
    var edit = id ? FW.db.getById(KEY, id) : null;
    var v = edit || { party: '', kind: '应收', category: '客户', date: FW.today(), amount: '', remark: '', settled: 0, project: '' };
    var body =
      '<div class="form-grid">' +
        '<div class="field"><label>对方名称（客户/供应商）</label><input id="c_party" value="' + FW.esc(v.party) + '" placeholder="如：XX公司"></div>' +
        '<div class="field"><label>类型</label><select id="c_kind"><option value="应收" ' + (v.kind === '应收' ? 'selected' : '') + '>应收（别人欠我）</option><option value="应付" ' + (v.kind === '应付' ? 'selected' : '') + '>应付（我欠别人）</option><option value="预付" ' + (v.kind === '预付' ? 'selected' : '') + '>预付（我预付给对方，待收回/核销）</option></select></div>' +
        '<div class="field"><label>关联项目（选填）</label><input id="c_project" value="' + FW.esc(v.project || '') + '" placeholder="如：XX项目，用于项目核算应收回款项"></div>' +
    ...
    FW.openModal(edit ? '编辑往来' : '新增往来', body, function () {
      ...
      document.getElementById('cSave').onclick = function () {
        ...
        var rec = {
          ...
          project: document.getElementById('c_project').value.trim(),
  ...
  function openSettle(id) {
    ...
    var remarkDef = r.party + (r.kind === '应收' ? ' 收款' : (r.kind === '应付' ? ' 付款' : ' 核销/收回'));
    var isPrepay = r.kind === '预付';
    var acctField = isPrepay ? '' : '<div class="field"><label>账户（仅登记内账用）</label><select id="s_acct">' + acctOpts + '</select></div>';
    var linkHtml = isPrepay ? '' :
      '<div class="field full"><label><input type="checkbox" id="s_link" checked style="width:auto;margin-right:6px">同时登记到「登记内账」流水（' + (r.kind === '应收' ? '收款记收入' : '付款记支出') + '）</label></div>';
    var body =
      '<div class="settle-info"> ' + FW.esc(r.party) + ' · ' + r.kind + (r.project ? ' · ' + FW.esc(r.project) : '') + ' · 余额 <b>' + FW.fmtMoney(remain) + '</b></div>' +
      '<div class="form-grid">' +
        '<div class="field"><label>本次核销/收回金额（元）</label><input id="s_amt" type="number" step="0.01" min="0" max="' + remain + '" value="' + remain + '"></div>' +
        '<div class="field"><label>日期</label><input id="s_date" type="date" value="' + FW.today() + '"></div>' +
        acctField +
        '<div class="field full"><label>备注</label><input id="s_remark" value="' + FW.esc(remarkDef) + '"></div>' +
        linkHtml +
      '</div>' +
    ...
    FW.openModal('核销 · ' + r.party, body, function () {
      ...
      document.getElementById('sSave').onclick = function () {
        ...
        FW.db.upsert(KEY, r);
        var linkEl = document.getElementById('s_link');
