...
(function (global) {
  ...
  function monthSum(m) {
    ...
    internalList().forEach(function (t) {
      if (t.date && t.date.slice(0, 7) === m) {
        ...
        // 退款支出（分类含「退款」）：冲减收入，不计入本月支出
        else if (t.type === 'expense') { if (FW.isRefundExpense && FW.isRefundExpense(t)) inc -= +t.amount; else exp += +t.amount; }
  ...
  function recentHtml(rows) {
    ...
    var trs = rows.map(function (t) {
      var cls = t.type === 'income' ? 'income' : (t.type === 'refund' ? 'refund' : (t.type === 'expense' ? ((FW.isRefundExpense && FW.isRefundExpense(t)) ? 'refund' : 'expense') : 'neutral'));
