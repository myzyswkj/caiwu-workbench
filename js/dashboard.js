...
(function (global) {
  ...
  function monthSum(m) {
    ...
    internalList().forEach(function (t) {
      if (t.date && t.date.slice(0, 7) === m) {
        ...
        else if (t.type === 'refund_out') inc -= +t.amount;   // 退款支出：冲减收入，不计入本月支出
  ...
  function recentHtml(rows) {
    ...
    var trs = rows.map(function (t) {
      var cls = t.type === 'income' ? 'income' : (t.type === 'refund' || t.type === 'refund_out' ? 'refund' : (t.type === 'expense' ? 'expense' : 'neutral'));
