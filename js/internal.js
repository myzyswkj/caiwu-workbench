...
(function (global) {
  ...
  function render() {
    ...
    if (tb) {
      ...
      var aliasB = document.getElementById('aliasBtn'); if (aliasB) aliasB.onclick = function () { if (FW.convenience) FW.convenience.openAliasManager(); };
      var ocrB = document.getElementById('ocrBtn'); if (ocrB) ocrB.onclick = function () { if (FW.convenience) FW.convenience.openOcrSettings(); };
  ...
  function drawList() {
    ...
    document.getElementById('fAllocStatus').value = f.allocStatus || '';
  ...
  function bindFilter() {
    ...
    g('fAllocStatus').onchange = function () { state.filter.allocStatus = this.value; drawTable(); };
    ...
    g('fReset').onclick = function () { state.filter = { project: '', category: '', category2: '', account: '', type: '', kw: '', from: '', to: '', noAlloc: false, allocStatus: '' }; drawList(); };
    ...
    if (type === 'transfer') {
      ...
      el.innerHTML =
        ...
        '<div class="form-grid">' +
          ...
          deductField +
        '</div>' +
        allocBoxHtml() + '<div class="skip-alloc"><label class="chk-inline"><input type="checkbox" id="f_skipAlloc"' + (v.skipAlloc ? ' checked' : '') + '> 不纳入分摊</label><div class="muted" style="font-size:12px;margin-top:3px">勾选后本笔从「仅看未分摊」隐藏，并标记为「不参与」，无需再人工判断是否要分摊。</div></div>';
  ...
  function openImport() {
    var body =
      '<div class="field"><label>导入方式</label><div class="seg">' +
        ...
        '<button type="button" class="seg-btn" data-m="paste">粘贴导入</button>' +
      '</div></div>' +
      '<div class="field" id="impFileWrap"><label>选择文件（CSV 或 Excel）</label><input type="file" id="impFile" accept=".csv,.xlsx,.xls,text/csv"></div>' +
      '<div class="field" id="impPasteWrap" style="display:none"><label>粘贴表格（Ctrl+V）</label><textarea id="impPaste" rows="8" placeholder="直接从 Excel / 网页表格复制后粘贴到这里；首行可放表头（日期 / 收支 / 金额 / 对方 / 备注）"></textarea></div>' +
      '<div class="field" id="impEncWrap"><label>编码</label><select id="impEnc"><option value="auto">自动（推荐）</option><option value="gbk">GBK（微信 / 老 Excel）</option><option value="utf8">UTF-8</option></select><span class="muted" style="font-size:12px;margin-left:6px">（仅 CSV 需要，Excel 自动识别）</span></div>' +
    ...
    FW.openModal('批量导入流水', body, function () {
      ...
      segs.forEach(function (b) {
        b.onclick = function () {
          ...
          var fw = document.getElementById('impFileWrap'); if (fw) fw.style.display = isPaste ? 'none' : '';
          var pw = document.getElementById('impPasteWrap'); if (pw) pw.style.display = isPaste ? '' : 'none';
          var ew = document.getElementById('impEncWrap'); if (ew) ew.style.display = isPaste ? 'none' : '';
          document.getElementById('impTip').textContent = mode === 'wechat'
            ? '微信账单：在微信「服务通知 / 钱包 → 账单 → 常见问题 → 下载账单 → 用于个人对账」导出 CSV（GBK）或 Excel（.xlsx）。'
            : (isPaste ? '粘贴导入：直接从 Excel / 网页表格复制后 Ctrl+V 粘贴到上方文本框，系统自动识别日期 / 金额 / 收支列。'
      ...
      document.getElementById('impParse').onclick = function () {
        if (mode === 'paste') {
          var txt = (document.getElementById('impPaste').value || '').trim();
          if (!txt) { FW.toast('请先粘贴表格内容'); return; }
          var lines = txt.split(/\r?\n/).filter(function (l) { return l.trim(); });
          var res = parseGenericCsv(txt, guessMap(csvSplit(lines[0] || '')));
          if (res.ok === false) { FW.toast(res.msg); return; }
          if (!res.rows.length) { FW.toast('没有可导入的记录（跳过 ' + res.skipped + ' 行）'); return; }
          FW.closeModal(); openImportPreview(res.rows, res.skipped, 'table');
          return;
        }
  ...
  function openForm(id, prefill) {
    ...
    if (prefill && !edit) {
      v.date = prefill.date || v.date; v.type = prefill.type || v.type;
      v.cat1 = prefill.cat1 || ''; v.cat2 = prefill.cat2 || '';
      v.account = prefill.account || v.account; v.amount = prefill.amount || '';
      v.remark = prefill.remark || ''; v.project = prefill.project || '';
    ...
    var body =
      '<div class="tx-form">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="form-actions"><button class="btn ghost" id="txDup">📋 复制上一笔</button><button class="btn ghost" id="txCancel">取消</button><button class="btn" id="txSave">保存</button></div>';
    ...
    FW.openModal(edit ? '编辑流水' : '新增流水', body, function () {
      ...
      var dupB = document.getElementById('txDup'); if (dupB) dupB.onclick = function () { if (FW.convenience) FW.convenience.dupLast(); };
      if (!edit && !prefill && FW.convenience) FW.convenience.maybeRestoreDraft();
      ...
      document.getElementById('txCancel').onclick = function () { unbind(); if (FW.convenience) FW.convenience.saveDraftFromForm(); FW.closeModal(); };
      document.getElementById('txSave').onclick = function () {
        ...
        var rec = {
          ...
          party: (FW.convenience ? FW.convenience.normalizeParty(document.getElementById('f_party').value.trim()) : document.getElementById('f_party').value.trim()),
        ...
        rec.skipAlloc = !!(document.getElementById('f_skipAlloc') && document.getElementById('f_skipAlloc').checked);
        unbind();
